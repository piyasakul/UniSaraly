...
begin
    ...
    begin
       ...
       // UniPDFFrame1.PdfURL := UniServerModule.LocalCacheURL + DosyaAdi;
        ...
        MessageDlg('ไม่พบไฟล์รายงานกรุณาตรวจสอบ', mtInformation, [mbOK]);
end;
...
begin
 {
    ...
    begin
          MessageDlg('สามารถขอข้อมูลย้อนหลังได้ไม่เกิน12 เดือน ',mtInformation,[mbOK]);
          Exit;
    end;
...

    {
       if Eof then
       begin
            MessageDlg('ไม่พบข้อมูลบุคลากรหรือข้อมูลยังไม่ครบถ้วน '+ #13 + 'ติดต่อกองคลัง 202',mtInformation,[mbOK]);
            Exit;
       end;
    ...
    begin
        ShowMessage('กรุณากำหนดเดือนป');
         //MessageDlg('กรุณากำหนดเดือนปี',mtInformation,[mbOK]);
        Abort;
    end;
    }
   // MessageDlg(YearEng(FormatDateTime('yyyymm',UniDateTimePicker1.DateTime)),mtInformation,[mbOK]);
    ...
    begin
       ...
       if Eof then
       begin
            MessageDlg('ไม่พบข้อมูลรายการรายได้หรือข้อมูลยังไม่ครบถ้วน '+ #13 + 'ติดต่อกองคลัง 202',mtInformation,[mbOK]);
            Exit;
       end;
       }
    end;
...
begin
  ...
  begin
    F:=Column.AuxValue;
    //Result:='รวมรายได้: '+FormatCurr('0.00 ', F) + FormatSettings.CurrencyString;
...
begin
      UniLabel7.Caption :=  'พัฒนาโดย: นายปิยะสกุล บรรลือวงศ์นักวิชการคอมพิวเตอร์ชำนาญการ สำนักปลัดฯ';
...
begin
      if MessageDlg('ยืนยันการลบข้อมูล'+
            #13+#10+'กดปุ่ม ok', mtConfirmation ,[mbNo,mbOK]) = mrNo then
          Abort;
...
begin
    ...
    begin
       MessageDlg('ไม่อนุญาตให้ลบข้อมูลข้อมูล' ,mtWarning,[mbOK]);
       Abort;
    end;
end;
...
begin
    ...
    begin
       MessageDlg('กรุณาตรวจสอบปีเดือน',mtInformation,[mbOK]);
       Abort;
    end;

    if  UniQuery3['money'] = Null  then
    begin
       MessageDlg('กรุณาระบุจำนวนเงิน',mtInformation,[mbOK]);
       Abort;
    end;

    if  UniQuery3['LookupLcode'] = null then
    begin
       MessageDlg('กรุณาระบุรายการ',mtInformation,[mbOK]);
       Abort;
    end;

    if UniQuery3['montid'] <  YearEng(FormatDateTime('yyyymm', Date)) then
    begin
       MessageDlg('ไม่อนุญาตให้ปรับข้อมูล' ,mtWarning,[mbOK]);
       Abort;
    end;
...
begin
    ...
    begin
       MessageDlg('ไม่อนุญาตให้แก้ไขข้อมูล' ,mtWarning,[mbOK]);
       Abort;
    end;
end;
...
begin
    ...
    begin
       MessageDlg('ไม่อนุญาตให้ลบข้อมูล',mtWarning,[mbOK]);
       Abort;
    end;
end;
...
begin
    ...
    begin
       MessageDlg('กรุณาระบุชื่อบุคลากร',mtInformation,[mbOK]);
       Abort;
    end;
...

 {
    ...
    begin
       MessageDlg('ชื่อบุคลากรที่เลือกมีในฐานข้อมูลแล้ว',mtInformation,[mbOK]);
       Abort;
    end;
  }
