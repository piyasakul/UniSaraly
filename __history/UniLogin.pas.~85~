...
begin
    ...
    begin
        ...
        begin
           try
             Server   := Ini.ReadString('SECURITY','DB_SERVER','localhost');
             Database := Ini.ReadString('SECURITY','DB_NAME','UdonpaoSalary');
             Username := Ini.ReadString('SECURITY','USERNAME','finance');
             Password := Ini.ReadString('SECURITY','PASSWORD','Udonpao@99');
             Port     := Ini.ReadInteger('SECURITY','DB_PORT',1433);
           ...
           try
            ...
            //  ShowMessage('ตรวจสอบข้อมูลเชื่อมต่อไม่สำเร็จ');
              Result := False;
           end;
        end;
    end
...
begin
    ...
    begin
       MessageDlg('กรุณาระบุผู้ใช้งานและรหัสผ่าน',mtInformation,[mbOK]);
       Abort;
    end;
    ...
    begin
      ...
      // ParamByName('pass').Value := aMd5.HashStringAsHex(UniEdit2.Text);
       ...
       begin
          ...
          MessageDlg('ไม่พบผู้ใช้งานหรือรหัสผ่านไม่ถูกต้อง',mtInformation,[mbOK]);
    end;
...
begin
    if ReadConnect then
       MessageDlg('เชื่อมต่อข้อมูลสำเร็จ',mtInformation, [mbOK])
    else
       MessageDlg('ตรวจสอบข้อมูลเชื่อมต่อไม่สำเร็จ',mtWarning,[mbOK]);


  {
     Confirm.Question('Confirma Grava็ใo','REGISTRO Nบ 584440',
    ...
    begin
      ...
      if Button = No then
        Confirm.Alert('','Nใo foi possivel concluir a grava็ใo entre em contato','', TTypeColor.orange, TTheme.modern);
        ...
        begin
             ...
             try
             {
              ...
              //  result:=True;
                ...
                MessageDlg('บันทึกการตั้งค่าฐานข้อมูลสำเร็จ', mtInformation, [mbOK]);
                ini.Free;
             end;
        end
         else begin
